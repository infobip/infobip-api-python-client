...
def test_get_number_masking_configuration_when_utc_date_is_returned_regardless_of_default_timezone(
    httpserver: HTTPServer, get_api_client
):
    original_timezone_class = datetime.timezone
    test_default_timezone = datetime.timezone(
        datetime.timedelta(hours=1)
    )  # Europe/Zagreb is UTC+1
    ...
    try:
        datetime.timezone = test_default_timezone
        response = api.get_number_masking_configurations()
        assert response == expected_response
    finally:
        # Restore the original datetime.timezone class
        datetime.timezone = original_timezone_class
