...
def test_should_get_voice_ivr_scenario(httpserver: HTTPServer, get_api_client):
    ...
    given_create_time_datetime = datetime.datetime(
        2023, 12, 6, 13, 37, 15, tzinfo=datetime.timezone.utc
    )
...
def test_should_create_call_api_voice_ivr_scenario(
    ...
    given_create_time_datetime = datetime.datetime(
        2024, 11, 9, 17, 0, 0, tzinfo=tzoffset(None, 3600)
    )

    setup_request(
        httpserver, "/voice/ivr/1/scenarios", given_response, "POST", 200, given_request
    )
...
def test_should_get_voice_ivr_scenario_by_id(httpserver: HTTPServer, get_api_client):
    ...
    given_create_time = "2023-12-06T13:37:15Z"
    given_create_time_datetime = datetime.datetime(
        2023, 12, 6, 13, 37, 15, tzinfo=tzoffset(None, 0)
    )
    given_update_time = None
    script = '[{"dial": "dial", "actionId": 1}]'
    ...
    setup_request(
        httpserver, f"/voice/ivr/1/scenarios/{given_id}", given_response, "GET", 200
    )
